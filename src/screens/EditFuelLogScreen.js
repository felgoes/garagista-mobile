...
import { updateFuelLog } from '../services/fuelLogs';
...
export default function EditFuelLogScreen({ route, navigation }) {
  const { vehicle, log } = route.params;

  const initialDate = isoToDisplay(log.date);
  const initialKmRaw = String(log.km);
  const initialLitersRaw = litersToRaw(log.liters);
  const initialPriceRaw = currencyToRaw(log.price_per_liter);
  const initialFullTank = log.full_tank;

  const [date, setDate] = useState(initialDate);
  const [kmRaw, setKmRaw] = useState(initialKmRaw);
  const [litersRaw, setLitersRaw] = useState(initialLitersRaw);
  const [priceRaw, setPriceRaw] = useState(initialPriceRaw);
  const [fullTank, setFullTank] = useState(log.full_tank);
  ...
  const isDirty =
    date !== initialDate ||
    kmRaw !== initialKmRaw ||
    litersRaw !== initialLitersRaw ||
    priceRaw !== initialPriceRaw ||
    fullTank !== initialFullTank;
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await updateFuelLog(vehicle.id, log.id, {
        date: parsedDate,
        km: kmNum,
        liters,
  ...
  const accentColor = consumption ? consumptionColor(consumption) : '#2563eb';
        ...
        >
          ...
          <View style={styles.kmCard}>
            ...
            <View style={styles.kmInputRow}>
              <TextInput
                ...
                value={maskKm(kmRaw)}
                onChangeText={(v) => setKmRaw(v.replace(/\D/g, ''))}
            ...
            <View style={styles.fieldBlock}>
              ...
              <View style={styles.maskedInputRow}>
                <TextInput
                  ...
                  placeholder="0,0"
                  placeholderTextColor="#cbd5e1"
                  value={litersRaw ? maskLiters(litersRaw) : ''}
          ...
          <View style={styles.maskedInputRow}>
            ...
            <TextInput
              ...
              placeholder="0,00"
              placeholderTextColor="#cbd5e1"
              value={priceRaw ? maskCurrency(priceRaw) : ''}
          ...
          {/* Consumo original do registro */}
          {consumption && (
            <View style={[styles.consumptionCard, { borderColor: accentColor }]}>
              <View style={[styles.consumptionIconBg, { backgroundColor: accentColor + '22' }]}>
                <MaterialCommunityIcons name="lightning-bolt" size={24} color={accentColor} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.consumptionValue, { color: accentColor }]}>
                  {consumption} km/L
                </Text>
                <Text style={[styles.consumptionBadge, { color: accentColor }]}>
...
const styles = StyleSheet.create({
  ...
  vehicleName: { fontSize: 13, fontWeight: '600', color: '#2563eb', marginBottom: 20 },
  kmCard: {
    backgroundColor: '#fff',
    borderRadius: 16,
    borderWidth: 2,
  ...
  kmInput: { fontSize: 52, fontWeight: '800', color: '#0f172a', minWidth: 120, textAlign: 'center', padding: 0 },
  ...
  consumptionIconBg: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
